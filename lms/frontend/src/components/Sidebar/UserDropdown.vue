				...
				>
					...
					>
						<div class="text-base font-medium text-ink-gray-9 leading-none">
							<span
								v-if="
									branding.data?.app_name && branding.data?.app_name != 'CIKKA'
								"
							>
	...
	return [
		{
			...
			items: [
				...
				},
				{
					...
					label: 'Login to CIKKA Cloud',
					onClick: () => {
						$dialog({
							title: __('Login to CIKKA Cloud?'),
							message: __(
								'Are you sure you want to login to your CIKKA Cloud dashboard?'
							),
							actions: [
								{
									label: __('Confirm'),
...
const clearDemoDataConfirmation = () => {
	$dialog({
		...
		message: __(
			'Are you sure you want to clear the demo data? This would delete the course "A guide  to CIKKA Learning" along with all its associated data. This action cannot be undone.'
		),
		actions: [
			{
				label: __('Confirm'),
