	...
	<div>
		<div class="mt-10 space-y-10">
			<div v-if="evals?.data?.length">
				...
				<div class="grid grid-cols-1 md:grid-cols-4 gap-5">
					<div
						v-for="(evaluation, index) in evals?.data"
						:key="evaluation.name || index"
						class="border hover:border-outline-gray-3 rounded-md p-3 flex flex-col h-full cursor-pointer transition-colors animate-fade-in-up"
			...
			<div v-if="liveClasses?.data?.length">
				...
				<div class="grid grid-cols-1 md:grid-cols-4 gap-5">
					<div
						v-for="(cls, index) in liveClasses?.data"
						:key="cls.name || index"
						class="border hover:border-outline-gray-3 rounded-md p-3 transition-colors animate-fade-in-up"
						:style="{ animationDelay: `${index * 60}ms` }"
					>
		...
		<div v-if="createdCourses.data?.length" class="mt-10">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
				<router-link
					v-for="(course, index) in createdCourses.data"
		...
		<div v-if="createdBatches.data?.length" class="mt-10">
			...
			<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
				<router-link
					v-for="(batch, index) in createdBatches.data"
