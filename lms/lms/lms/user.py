...
# Only email addresses ending with this domain are allowed to sign up / log in.
# Administrator and System Users (staff/admins) are exempt from this restriction.
ALLOWED_EMAIL_DOMAIN = "@customerik.com"


def is_email_domain_allowed(email: str) -> bool:
	"""Return True if the email belongs to the allowed organisation domain."""
	if not email:
		return False
	return email.strip().lower().endswith(ALLOWED_EMAIL_DOMAIN)


def is_domain_restriction_exempt(user: str) -> bool:
	"""Administrator and System Users keep their access regardless of domain."""
...
def restrict_login_domain(login_manager):
	"""Block login for non-organisation emails. Runs before the session is created.

	Administrator and System Users are left untouched so admin login is unchanged.
	"""
	user = login_manager.user
	if is_domain_restriction_exempt(user):
		return
	if not is_email_domain_allowed(user):
		frappe.throw(
			_("Login is restricted to {0} email addresses only.").format(ALLOWED_EMAIL_DOMAIN),
...
def sign_up(email: str, full_name: str, verify_terms: bool, user_category: str):
	...
	if not is_email_domain_allowed(email):
		frappe.throw(
			_("Sign Up is restricted to {0} email addresses only.").format(ALLOWED_EMAIL_DOMAIN),
			_("Not Allowed"),
		)
...
def on_login(login_manager):
	restrict_login_domain(login_manager)
